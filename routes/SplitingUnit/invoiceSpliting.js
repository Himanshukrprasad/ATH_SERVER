...
router.post("/", async (req, res) => {
  try {
    ...
    const {designNo} =  splitingCompData.splitingValues[0].designNo;
  ...
  calValues[i] = calculatedValue;
    ...
    try {
      ...
      const insertDestinationQuery =
        "INSERT INTO spliting_destination (master_id, alter_id, vch_no, vch_date, item_name, godown, design_no, design_id ,type, size,lot_no,quantity,rate, amt) VALUES (?,?, ?,?, ?, ?,? , ?, ?, ?,?,?,?,?)";
      if (
        splitingCompData.destinationTableData &&
        splitingCompData.destinationTableData.length
      ) {
      ...
      const insertSourceQuery =
        "INSERT INTO spliting_source  (master_id, alter_id, vch_no, vch_date,amt, item_name, godown, design_no,design_id, size,lot_no,quantity,rate ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
      await connection.query(insertSourceQuery, [
        ...
        splitingCompData.splitingValues[0].designNo || "NA",
        splitingCompData.mainGodown[0].godown || "NA",
        splitingCompData.splitingValues[0].designNo || "NA",
        designIdData || "NA",
        splitingCompData.splitingValues[0].actualSize || "NA",
        splitingCompData.destinationData[0].lot_no || "NA",
