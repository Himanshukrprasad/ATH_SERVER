  ...
  try {
   ...
   const {allocationToPri, cuttingCharges, effectiveCost, effectiveRate, freightCharges, totalAdlCost,componentCost} = req.body.billingUnitData;
    ...
    try {
      ...
      let lastVchNo = lastVchNoResult[0]?.vch_no || "C/U1/000/24/25";
      ...
      const tem_vch = `C/U${cuttingUnitNo}/${temVchNo}/${financialYear}`;
      const result = (estimatedQty - (totalRemQty + actualQty)) || 0;

      // Insert header data
      const insertHeaderQuery = "INSERT INTO cutting_header (master_id, alter_id, vch_no, estimate_cs_vch_no, vch_date,unit_no,jobber_id,jobber_name,design_id,design_no,  estimated_qty, actual_qty, remaining_qty, lot_no, size,other_cost_type, other_cost_val, freight_one_type, freight_one_val, freight_two_type, freight_two_val, godown, Narration) VALUES (?,?,?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?)";
