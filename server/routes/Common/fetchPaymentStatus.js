...
router.post("/", async (req, res) => {
    let { jobberName, designNo, process } = req.body;
    let billDone = 0;
  
    try {
      // Fetch vch_no from purchase_bill_detail table where design_no === designNo
      const [vchResult] = await pool.query(
        "SELECT vch_no FROM purchase_bill_detail WHERE design_no = ?",
        [designNo]
      );
