...
//                 // Manually construct the query string to include the table name
//                 const query = `SELECT * FROM ${connection.escapeId(table)} WHERE vch_no = ?`;
...
//                 results.forEach((row) => {
//                     row = formatNumbers(row); // Format numbers for the table
//                     searchResults.push({
//                         tableName: table,
//                         data: row
//                     });
...
router.post('/', async (req, res) => {
    try {
        ...
        try {
            ...
            const headerQuery = `SELECT vch_no FROM Estimated_CS_header WHERE design_no = ?`;
