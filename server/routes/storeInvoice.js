...
router.post("/", async (req, res) => {
  try {
    ...
    try {
      ...
      let lastVchNo = lastVchNoResult[0]?.vch_no || "EC/24-25/0000";

      console.log('last vch no', lastVchNoResult)
      
  
      // Extract the numeric part after 'EC-23-24-'
      const lastVchNoNumericPart = parseInt(lastVchNo.split('EC/24-25/')[1]);
      
      // Increment the numeric part and format it back
      const newLastDigits = (lastVchNoNumericPart + 1).toString().padStart(4, '0');
      const tem_vch = `EC/24-25/${newLastDigits}`;
