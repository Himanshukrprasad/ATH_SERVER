...
router.post("/", async (req, res) => {
  try {
    const { semiFinishedGoodsContextUnitData } = req.body;
    let embIssNoText = `SFG/${semiFinishedGoodsContextUnitData.lastEmbNo}/${semiFinishedGoodsContextUnitData.embUnitData[0].embUnitVal}/24-25`;
    let unitNO = `SEMI FINISHED GOODS -${semiFinishedGoodsContextUnitData.embUnitData[0].embUnitVal}`
// Filter values starting with 'EC'
const filteredValues = semiFinishedGoodsContextUnitData.esVchNoVal.filter(value => value.startsWith('EC'));
...
const filteredString = filteredValues.join(',');
    ...
    try {
      ...
      await connection.query(insertHeaderQuery, [
        ...
        JSON.stringify(semiFinishedGoodsContextUnitData.esVchNoVal) || 'NA',
    JSON.stringify(semiFinishedGoodsContextUnitData.designNoVal) || 'NA',
    semiFinishedGoodsContextUnitData.designIdVal || 'NA',
    JSON.stringify(semiFinishedGoodsContextUnitData.lotNoVal) || 'NA',
      ...
      if (
        ...
        for (let i = 0; i < semiFinishedGoodsContextUnitData.destinationTableData2.length; i++) {
          await connection.query(insertDestinationQuery, [
            ...
            semiFinishedGoodsContextUnitData.destinationTableData2[i].designNo || 'NA',
            semiFinishedGoodsContextUnitData.designIdVal || 'NA',
      ...
      const insertSourceQuery =
        ...
        if (
          ...
          for (let i = 0; i < semiFinishedGoodsContextUnitData.sourceTableData.length; i++) {
            await connection.query(insertSourceQuery, [
              ...
              semiFinishedGoodsContextUnitData.sourceTableData[0].design_no || 'NA',
              semiFinishedGoodsContextUnitData.designIdVal || 'NA',
              semiFinishedGoodsContextUnitData.sourceTableData[i].godown || 'NA',
              semiFinishedGoodsContextUnitData.sourceTableData[0].size || 'NA',
              semiFinishedGoodsContextUnitData.sourceTableData[0].lot_no || 'NA',
        ...
        "UPDATE spliting_header SET last_operation = 'SFG' WHERE estimated_cs_vch_no = ?";
