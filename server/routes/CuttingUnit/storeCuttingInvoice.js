  ...
  try {
   ...
   const {allocationToPri, cuttingCharges, effectiveCost, effectiveRate, freightCharges, totalAdlCost,componentCost} = req.body.billingUnitData;
    ...
    try {
      ...
      let lastVchNo = lastVchNoResult[0]?.vch_no || "C/U1/0000/24/25";
      ...
      const tem_vch = `C/${temVchNo}/${cuttingUnitNo}/${financialYear}`;
...
const insertRowItem = "INSERT INTO cutting_item_detail (master_id, alter_id, vch_no, estimate_cs_vch_no, vch_date,item_id,item_name, godown, type, size, lot_no, tolerance, qty, rate,uom, amt) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
if (consumptionUnitData && consumptionUnitData.length) { 
    for (let i = 0; i < consumptionUnitData.length; i++) {
        await connection.query(insertRowItem, [
            ...
            consumptionUnitData[i].unit || 'N/A',
