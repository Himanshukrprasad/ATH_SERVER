...
router.get('/', (req, res) => {
  ...
  pool.query(query, (error, results) => {
    ...
    if (lastLotNo) {
      ...
      newLotNo = numericPart.toString().padStart(3, '0');
    } else {
    ...
    if (lastVchNo) {
      ...
      const vchNumericPart = parseInt(lastVchNo.split('/')[1]) + 1;
      // Format it to ensure it has four digits
      newVchNoSuffix = vchNumericPart.toString().padStart(3, '0');
    } else {
    ...
    console.log('lll',{ lot_no: newLotNo, vch_no_suffix: newVchNoSuffix } )
  });
});
